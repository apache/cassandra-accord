...
public class KnownMap extends ReducingRangeMap<Known>
{
    ...
    {
        Known known = owns.isEmpty() ? knownForAny() : validForAll.atLeast(foldlWithDefault(owns, KnownMap::reduceKnownFor, Known.Nothing, null, i -> false));
        if (owns != touches && !touches.isEmpty())
        {
            Known knownDeps = validForAll.atLeast(foldlWithDefault(touches, KnownMap::reduceKnownFor, Known.Nothing, null, i -> false));
