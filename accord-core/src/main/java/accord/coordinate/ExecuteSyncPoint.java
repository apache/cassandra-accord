...
public class ExecuteSyncPoint extends AbstractCoordination<Route<Range>, DurabilityResult, ReadReply, Void> implements Callback<ReadReply>
{
    ...
    {
        SortedArrayList<Node.Id> readable = this.readable.toSortedArrayList(Node.Id[]::new);
        SortedArrayList<Node.Id> including = this.including.toSortedArrayList(Node.Id[]::new);
        DurabilityResult cur = new DurabilityResult(syncPoint,
                                                    tracker.results(node.id(), knownToSelf, inflightCount() > slow.size(), readable),
                                                    including,
                                                    readable,
                                                    failure()
        );
