...
public interface CoordinationAdapter<R>
{
    ...
    void persist(Node node, Topologies all, Topologies executes, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, BiConsumer<? super R, Throwable> callback);
    ...
    class Invoke
    {
        ...
        public static <R> void persist(CoordinationAdapter<R> adapter, Node node, Topologies any, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, @Nullable BiConsumer<? super R, Throwable> callback)
        {
            ...
            adapter.persist(node, any, executes, route, txnId, txn, executeAt, deps, writes, result, callback);
        }
        ...
        public static <R> void persist(CoordinationAdapter<R> adapter, Node node, FullRoute<?> route, Participants<?> sendTo, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, @Nullable BiConsumer<? super R, Throwable> callback)
        {
            ...
            adapter.persist(node, all, executes, route, txnId, txn, executeAt, deps, writes, result, callback);
        }
    }
    ...
    class Adapters
    {
        ...
        public static class StandardTxnAdapter extends AbstractTxnAdapter
        {
            ...
            @Override
            public void persist(Node node, Topologies all, Topologies executes, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, BiConsumer<? super Result, Throwable> callback)
            {
                if (callback != null) callback.accept(result, null);
                new PersistTxn(node, all, txnId, route, txn, executeAt, deps, writes, result)
                    .start(Apply.FACTORY, Minimal, all, executes, writes, result);
            }
        }
        ...
        public static class RecoveryTxnAdapter extends AbstractTxnAdapter
        {
            ...
            public void persist(Node node, Topologies all, Topologies executes, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, BiConsumer<? super Result, Throwable> callback)
            {
                if (callback != null) callback.accept(result, null);
                new PersistTxn(node, all, txnId, route, txn, executeAt, deps, writes, result)
                    .start(Apply.FACTORY, Maximal, all, executes, writes, result);
            }
        }
        ...
        static abstract class AbstractSyncPointAdapter<S extends Seekables<?, ?>> implements CoordinationAdapter<SyncPoint<S>>
        {
            ...
            @Override
            public void persist(Node node, Topologies all, Topologies executes, FullRoute<?> route, TxnId txnId, Txn txn, Timestamp executeAt, Deps deps, Writes writes, Result result, BiConsumer<? super SyncPoint<S>, Throwable> callback)
            {
