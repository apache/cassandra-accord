...
public class ExecuteExclusiveSyncPoint extends ReadCoordinator<ReadReply>
{
    @SuppressWarnings("unused")
    private static final Logger logger = LoggerFactory.getLogger(ExecuteExclusiveSyncPoint.class);
    ...
    ExecuteExclusiveSyncPoint(Node node, Topologies topologies, FullRoute<?> route, TxnId txnId, Txn txn, long executionEpoch, Deps deps, BiConsumer<? super Result, Throwable> callback)
    {
    ...
    protected Action process(Id from, ReadReply reply)
    {
        if (reply.isOk())
        {
            ...
            if (ok.futureEpoch > allTopologies.currentEpoch())
            {
                ...
                new ExecuteExclusiveSyncPoint(node, node.topology().preciseEpochs(route, ok.futureEpoch, ok.futureEpoch), route, txnId, txn, ok.futureEpoch, deps, callback).start();
                return Aborted;
            }
